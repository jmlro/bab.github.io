...
# {{ post_year }}
{% endif %}{% assign mes_num = post.date | date: "%-m" | minus: 1 %}{{ post.date | date: "%d" }} {{ meses[mes_num] }}   <a href="{{ post.url }}">{{ post.title }}</a> [<a href="/category/{{ post.categories | first }}/">{{ post.categories | first }}</a>]
{% endfor %}
